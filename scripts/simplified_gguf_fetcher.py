...
class SimplifiedGGUFetcher:
    ...
    def _generate_output(self, processed_models: List[Dict]) -> None:
        """
        Generate JSON array with exactly 16 fields per model (10 original + 1 capability + 4 hardware requirements + 1 upload date).
        ...
        try:
            ...
            if output_models:
                ...
                for model in output_models:
                    cap = model.get('modelCapability', 'text')
                ...
                self.logger.info(f"Model capability distribution:")
                for cap, count in sorted(capability_counts.items(), key=lambda x: -x[1]):
                    percentage = (count / len(output_models)) * 100
                    self.logger.info(f"  - {cap}: {count} ({percentage:.1f}%)")
    ...
    def _detect_model_capability(self, model_id: str, tags: List[str], model_name: str = '') -> str:
        ...
        Args:
            ...
            model_name: Optional model name for additional pattern matching
            
        Returns:
            Model capability string: 'vision', 'embedding', 'code', 'audio', or 'text'
        """
        if not model_id:
            return "text"
        
        # Combine model_id and model_name for comprehensive matching
        search_text = f"{model_id} {model_name}".lower()
        tags_lower = [tag.lower() for tag in (tags or [])]
        all_text = search_text + ' ' + ' '.join(tags_lower)
        
        # Vision/Multimodal patterns
        vision_patterns = [
            'vision', 'vl', '-v-', '-v1', '-v2', 'image', 'llava', 'cogvlm', 
            'minicpm-v', 'qwen-vl', 'qwen2-vl', 'internvl', 'idefics', 
            'fuyu', 'paligemma', 'moondream', 'bakllava', 'obsidian',
        ...
        vision_tags = ['image-text-to-text', 'vision', 'multimodal', 'image-to-text', 'visual-question-answering']
        for tag in tags_lower:
            if tag in vision_tags or 'vision' in tag or 'visual' in tag:
                return "vision"
        
        # Embedding patterns
        embedding_patterns = [
            'embed', 'embedding', 'bge-', 'bge_', 'e5-', 'gte-', 
            'nomic-embed', 'sentence-', 'all-minilm', 'instructor',
            'retrieval', 'dense-passage', 'contriever', 'colbert'
        ]
        for pattern in embedding_patterns:
            if pattern in all_text:
                return "embedding"
        
        # Check tags for embedding indicators
        embedding_tags = ['sentence-similarity', 'feature-extraction', 'sentence-transformers']
        for tag in tags_lower:
            if tag in embedding_tags or 'embed' in tag:
                return "embedding"
        
        # Code patterns
        code_patterns = [
            'code', 'coder', 'codellama', 'starcoder', 'deepseek-coder',
            'codegen', 'santacoder', 'replit-code', 'wizardcoder',
            'phind-code', 'magicoder', 'octocoder', 'codebooga',
            'sqlcoder', 'nsql', 'defog'
        ]
        ...
        # Check tags for code indicators
        code_tags = ['code', 'text-generation']
        for tag in tags_lower:
            if 'code' in tag or 'programming' in tag:
                return "code"
        
        # Audio/Voice patterns
        audio_patterns = [
            'whisper', 'audio', 'speech', 'tts', 'voice', 'wav2vec',
            'hubert', 'speecht5', 'bark', 'musicgen', 'audiogen',
            'seamless', 'mms-', 'asr', 'stt', 'text-to-speech',
        ...
        # Check tags for audio indicators
        audio_tags = ['automatic-speech-recognition', 'text-to-speech', 'audio-to-audio', 'text-to-audio']
        for tag in tags_lower:
            if tag in audio_tags or 'audio' in tag or 'speech' in tag:
                return "audio"
        
        # Default to text/chat for general LLMs
        return "text"
  ...
  %(prog)s --disable-spam-filter  # Run without spam filtering (basic GGUF filtering only)
    ...
    if not args.disable_spam_filter:
        filter_config = FilterConfig(
            ...
            backup_enabled=False,  # Disabled to save storage space
