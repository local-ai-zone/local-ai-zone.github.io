...
class SimplifiedGGUFetcher:
    ...
    def _deduplicate_across_repos(self, models: List[Dict]) -> List[Dict]:
        """
        Deduplicate models across different HuggingFace repos — but keep ALL
        GGUF files per model.
        
        When the same base model (e.g., Qwen3-Coder-Next) is uploaded to multiple
        repos, keep only the entries from the repo with the highest engagement
        (likes + downloads), but retain every GGUF file of that winning repo
        (Q4_K_M, Q8_0, F16, ...) so the UI can offer a file/quantization
        selector instead of hiding all but one file.
        
        Args:
            models: List of processed model entries (after _extract_model_info)
            
        Returns:
            Deduplicated list with every file of each unique base model
        """
        if not models:
            return models
        ...
        for canonical_name, group in groups.items():
            # Multiple repos may have this model - pick the winning REPO
            ...
            duplicates_removed += len(group) - len(winner_files)
        
        if duplicates_removed > 0:
            self.logger.info(
                f"Cross-repo deduplication: kept {len(deduplicated)} entries "
                f"(removed {duplicates_removed} duplicate repos)"
            )
    ...
    def _extract_model_info(self, model_data: Dict) -> List[Dict]:
        ...
        processed_entries = self._aggregate_shard_sizes(processed_entries)
    ...
    def _aggregate_shard_sizes(self, entries: List[Dict]) -> List[Dict]:
        ...
        for (model_id, base_name), group in groups.items():
            ...
            kept['fileSize'] = total
            kept['fileSizeFormatted'] = self._format_file_size(total)
