...
class ModelCard {
    ...
    _generateCardHTML() {
        ...
        const modelCapability = this.data.modelCapability || 'text';
        ...
        return `
            ...
            <div class="model-details">
                ...
                <span class="detail-value">
                    ${this._generateCapabilityBadge(modelCapability)}
                </span>
    ...
    /**
     * Generate capability badge HTML for model capability type
     * @param {string} capability - Model capability (vision, embedding, code, audio, text)
     * @returns {string} HTML string for capability badge
     * @private
     */
    _generateCapabilityBadge(capability) {
        const capabilityConfig = {
            vision: { icon: '👁️', color: '#8B5CF6' },
            embedding: { icon: '🔗', color: '#06B6D4' },
        ...
        return `
            <div class="capability-badge" 
                 style="--capability-color: ${config.color}"
    ...
    matchesFilters(filters) {
        ...
        // Check model capability filter
        if (filters.modelCapability && filters.modelCapability !== 'all') {
            if ((this.data.modelCapability || 'text') !== filters.modelCapability) {
                return false;
            }
        }
